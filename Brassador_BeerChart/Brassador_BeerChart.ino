...
// Version
#define BVERSION "0.0.2"

// Setups
#define BUTTON_DELAY 100  // Sensitivity adjustment
...
void setup() {
  ...
  // Temperature was set for 97 degrees because of altitude - 100C might not be reachable
  pre_heat(97);
  brew(100, 60);
...
int get_user_parameter(String prefix)
{
  ...
  {      
    ...
    button_value = analogRead(1);
    if (button_value > 1000 && button_value < 1005)
    {
      temp++;
    }
    else if (button_value > 1010 && button_value < 1020)
    {      
      if (temp > 0)
      {
        temp--;        
  ...
  while (button_value < 980 || button_value > 995);
